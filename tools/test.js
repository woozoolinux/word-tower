...
// 적에게 체력을 준다 — 한 방에 죽으면 맞히는 맛이 없다
ok('적이 총알 여러 발을 버틴다', ARB.foeHp >= 2, ARB.foeHp + '발');
// 적이 내 줄까지 와서 싸울 시간이 남아야 한다. 웨이브 시간을 다 써서 오면 도착하자마자 끝난다
ok('적이 도착한 뒤에도 싸울 시간이 남는다', ARB.foeSpeed < 0.85, '이동에 ' + Math.round(ARB.foeSpeed * 100) + '%');
...
// 돌벽이 있고 그 안에 금고 문이 박혀 있어야 꽂고 싶어진다.
// ===================================================================
const VC = fs.readFileSync(path.join(ROOT, 'css/style.css'), 'utf8');
ok('금고 방에 벽이 있다', /#screen-vault\s*\{[^}]*repeating-linear-gradient/.test(VC));
ok('금고 방에 바닥이 있다', /#screen-vault::after\s*\{/.test(VC));
ok('자물쇠가 철판에 박혀 있다', /\.vault-locks\s*\{[^}]*inset 0 0 0 3px/.test(VC));
ok('카드가 선반 위에 놓인다', /\.vault-cards\s*\{[^}]*box-shadow/.test(VC));
// 횃불을 철판 **바깥**에 두면 화면 가장자리에서 잘린다. 실제로 반이 잘렸다
ok('금고 옆 횃불이 화면 안에 있다', (() => {
  const m = VC.match(/\.vault-locks::before \{ left: (-?\d+)px; \}/);
  const m2 = VC.match(/\.vault-locks::after\s+\{ right: (-?\d+)px;/);
  return !!m && !!m2 && Number(m[1]) >= 0 && Number(m2[1]) >= 0;
})());
