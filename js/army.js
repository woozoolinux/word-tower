...
const Army = (() => {
  ...
  let bg, crowd, foes, shots, wave, result, runWords, msg, msgT;
  ...
  function gateStep(n, ok) {
    const a = A();
    return Math.min(a.cap, Math.max(a.min, Math.round(ok ? n * a.win : n * a.lose)));
  }
  ...
  function waveOutcome(n, g, size) {
    const kills = Math.min(size, firepower(n, g));
    return { kills, loss: Math.min(A().maxLoss, size - kills) };
  }
  ...
  function simulate(mask) {
    const a = A();
    let n = a.start, g = 0;
    for (let i = 0; i < a.gates; i++) {
      const ok = typeof mask === 'number' ? !!(mask & (1 << i)) : i < mask;
      n = gateStep(n, ok);
      if (ok) g++;
      n = Math.max(a.min, n - waveOutcome(n, g, a.waves[i]).loss);
    }
    return { troops: n, power: firepower(n, g), lv: level(g), win: firepower(n, g) >= a.boss };
  }
  ...
  function reset() {
    const a = A();
    troops = a.start; wgauge = 0; gateIdx = 0; lane = -1; laneS = -1;
  ...
  function buildQueue() {
    ...
    queue = picks.map(w => {
      ...
      const o = other.w || other;
      const ansRight = Math.random() < .5;
      return { word: w, ans: ansRight ? 1 : -1, left: ansRight ? o : w.w, right: ansRight ? w.w : o };
    });
    // 같은 쪽이 셋 넘게 이어지면 아이가 규칙을 눈치챈다
    for (let i = 2; i < queue.length; i++) {
      if (queue[i].ans === queue[i - 1].ans && queue[i].ans === queue[i - 2].ans) {
        queue[i].ans = -queue[i].ans;
        const t = queue[i].left; queue[i].left = queue[i].right; queue[i].right = t;
      }
    }
  ...
  function brief() {
    ...
    UI.modal(`
      ...
      <div class="modal-sub">끝에는 <b>적장의 무리 ${a.boss}명</b>.
  ...
  function nextGate() {
    if (gateIdx >= queue.length) { startWave(A().boss, true); return; }
  ...
  function fight(dt) {
    const a = A();
    phaseT += dt;
    const dur = wave.boss ? a.bossTime : a.waveTime;
    ...
    while (wave.killed < want && guard++ < 12) {
      ...
      shots.push({ x0: laneS * 0.5 + (Math.random() - .5) * .5, z0: LINE, tx: target.ox, tz: target.z, t: 0, f: target });
      wave.killed++;
    }
    shots.forEach(s => {
      s.t += dt / (s.spark ? a.shotTime * 1.6 : a.shotTime);
    ...
    if (!wave.applied && phaseT > dur * 0.86) {
      wave.applied = true;
      if (wave.loss > 0) {
        troops = Math.max(a.min, troops - wave.loss);
        Sfx.bad(); hit = .45; say('−' + wave.loss + '명', '#ff8090');
      } else if (!wave.boss) say('막아냈다!', '#3ee0c4');
      setAsk(wave.boss ? '적장의 무리!' : '막아라!', wave.boss ? '마지막 싸움' : '적이 내려온다');
    }
    if (phaseT > dur + 0.6) {
  ...
  function drawGate(g) {
    const z = Math.max(0, g.z), s = sz(z), y = pz(z), hgt = 78 * s;
    [-1, 1].forEach(side => {
      ...
      ctx.fillText(side < 0 ? g.left : g.right, cx, y - hgt / 2, w - 8 * s);
    });
  }
  ...
  function drawCrowd() {
    ...
    const sp = spread(troops), mid = phase === 'gate' ? laneS * 0.5 : 0;
  ...
  function won(power) {
    ...
    UI.modal(`
      ...
      <div class="modal-sub">부하 <b>${troops}명</b>이 ${wp.emoji} <b>${wp.name}</b>으로<br>
        적장의 무리 <b>${a.boss}명</b>을 쓸어버렸어요!</div>
  ...
  function lost(power) {
    ...
    UI.modal(`
      ...
      <div class="modal-sub">화력이 <b>${power}</b>밖에 안 됐어요.<br>
  ...
  return { start, stop, simulate, gateStep, firepower, level, waveOutcome, WEAPONS, debug };
})();
