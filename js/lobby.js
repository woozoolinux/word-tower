...
const Lobby = (() => {
  ...
  function zoneCard(z) {
    const lock = zoneLock(z), locked = !!lock;
    ...
    return `<div class="zone ${locked ? 'locked' : 'panel'}" data-zone="${z.id}">
      ${locked ? `<span class="lock-badge">${lock.tag}</span>` : ''}
  ...
  function enterZone(id) {
    const z = ZONES.find(x => x.id === id), lock = zoneLock(z);
    if (lock) {
      // 날개가 없어 못 가는 건 **안내가 아니라 목표**다. 그림으로 보여준다.
      if (lock.kind === 'aura' && id === 'sky') { SkyIsland.needWings(); return; }
      UI.toast(`🔒 ${z.name} — ${lock.text}`); Sfx.bad(); return;
    }
