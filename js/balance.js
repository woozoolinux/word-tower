...
const BAL = {
  ...
  // ⚔️ 연병장 — 두 갈래 문 중 맞는 쪽을 통과하면 부하가 불어난다.
  //
  // 왜 곱하기인가: 맞을 때 +N, 틀릴 때 −N 으로 하면 **몇 번째 문에서 틀렸는지**에 따라
  // 결과가 달라진다. 곱하기는 순서가 상관없다 (곱셈은 자리를 바꿔도 같다) —
  // "몇 개 맞혔나" 하나로 결과가 정해지니 난이도를 테스트로 못 박을 수 있다.
  //
  // 그리고 곱하기라야 마지막에 수백 명이 된다. 그 순간이 이 게임의 전부다.
  army: {
    start: 8,           // 처음 부하 수
    gates: 6,           // 문 = 문제 수
    win: 2,             // 맞으면 몇 배
    lose: 0.55,         // 틀리면 몇 배
    min: 2,             // 아무리 틀려도 이만큼은 남는다 (0이 되면 판이 끝나 버린다)
    // 적군은 "4개 맞히면 이기고 3개면 진다"에 맞춰 자동으로 정해진다.
    // 4맞2틀 = start · win⁴ · lose²  →  거기서 조금 덜어낸 수
    enemyEase: 0.62,
    drawMax: 72,        // 화면에 실제로 그리는 부하 수. 나머지는 숫자로 (폰이 버텨야 한다)
    approach: 2.4,      // 문 하나가 다가오는 시간(초)
    words: 30,          // 후보 단어 — 마지막으로 오른 탑에서, 없으면 본 적 있는 것 중
    gold: { win: 40, perTroop: 0.15, lose: 8 },
    exp: { win: 36, lose: 10 },
  },
