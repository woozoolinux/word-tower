...
const BAL = {
  ...
  army: {
    ...
    gateBase: 10,       // 문을 지나면 기본으로 이만큼 (오답 문이면 이만큼 잃는다)
    gateBonus: 3,       // 문이 오는 내내 겨누고 있으면 여기까지 더 붙는다
    min: 1,             // 아무리 잃어도 이만큼은 남는다
    cap: 90,            // 부하 상한
    fire: 1.1,          // 병사 하나가 한 웨이브에서 잡는 적 수 (화력 = 병사 × 이것)
    waves: [3, 6, 10, 16, 24],    // 문을 지날 때마다 내려오는 적
    maxLoss: 3,         // 한 웨이브가 데려갈 수 있는 병사 (죽음의 나선을 막는 빗장)
    boss: 40,           // 마지막 적장의 무리 — 화력이 이만큼 되면 이긴다
                        // (5맞 최소 화력 56, 2맞 최대 28 — 그 사이)
    ...
    gold: { win: 40, perTroop: 0.6, lose: 8 },
    exp: { win: 36, lose: 10 },
  },
