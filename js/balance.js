...
const BAL = {
  ...
  army: {
    ...
    weaponMax: 3,       // 🪨 돌팔매 → 🏹 화살 → 🔥 불화살 → ⚡ 번개
    weaponPer: 1,       // 문 하나 맞힐 때마다 무기 게이지 +1, 이만큼마다 한 단계
    fire: [0.5, 0.8, 1.1, 1.5],   // 무기 단계별 — 병사 하나가 한 웨이브에서 잡는 적 수
    waves: [6, 12, 20, 30, 42],   // 문을 지날 때마다 내려오는 적
    maxLoss: 3,         // 한 웨이브가 데려갈 수 있는 병사 (죽음의 나선을 막는 빗장)
    boss: 56,           // 마지막 적장의 무리 — 화력이 이만큼 되면 이긴다
    ...
    tierRush: 0.32,     // 한 단계마다 문이 다가오는 시간이 이만큼 준다(초)
    minApproach: 1.6,   // 아무리 빨라도 이보다 짧아지진 않는다
    drawMax: 50,        // 화면에 실제로 그리는 수. 나머지는 숫자로 (폰이 버텨야 한다)
    approach: 2.6,      // 문이 다가오는 시간(초)
