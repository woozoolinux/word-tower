...
const BAL = {
  ...
  // ⚔️ 연병장 — 문을 지나며 **계속 싸운다.**
  //
  // 왜 게이트가 전부 곱하기인가: 맞을 때 +N, 틀릴 때 −N 으로 하면
  // **몇 번째 문에서 틀렸는지**에 따라 결과가 달라진다. 곱하기는 순서가 상관없다.
  //
  // 왜 전투 손실이 머릿수 상한인가: 비율(예: 30%)로 두면 병력이 적을 때
  // 배로 늘려도 그만큼 다시 잃어서 영영 못 올라온다 — **죽음의 나선**.
  // 상한을 머릿수로 두면 작은 부대도 ×2 두 번이면 되살아난다.
  //
  // 지키는 선은 하나: **4개 맞히면 이기고 3개면 진다.**
  ...
  army: {
    ...
    win: 2,             // 맞으면 몇 배
    lose: 0.6,          // 틀리면 몇 배
    min: 6,             // 아무리 잃어도 이만큼은 남는다 (0이면 판이 끝나 버린다)
    cap: 80,            // 부하 상한 — 화면에도 폰에도 이 이상은 무리다
    // 무기는 **하나뿐**이다. 총알은 병사 수에 비례할 뿐 —
    // 많이 맞혀서 사람이 많아지면 그만큼 총알이 많이 나간다. 그게 보상이다.
    // 무기 사다리(화살→불화살→번개)를 두면 "몇 개 맞혔나" 위에 층이 하나 더 얹혀
    // 아이가 무엇 때문에 세졌는지 흐려진다
    fire: 1.1,          // 병사 하나가 한 웨이브에서 잡는 적 수
    waves: [6, 12, 20, 30, 42],   // 문을 지날 때마다 내려오는 적
    maxLoss: 3,         // 한 웨이브가 데려갈 수 있는 병사 (죽음의 나선을 막는 빗장)
    boss: 40,           // 마지막 적장의 무리 — 화력이 이만큼 되면 이긴다
                        // (3맞 화력 최대 29, 4맞 최소 52 — 그 사이)
    ...
    tierPer: 8,         // 몇 레벨마다 한 단계
    tierMax: 4,         // 아무리 높아도 이 이상은 안 커진다
    tierRush: 0.3,      // 한 단계마다 문이 다가오는 시간이 이만큼 준다(초)
    minApproach: 2.8,   // 아무리 빨라도 이보다 짧아지진 않는다
    drawMax: 50,        // 화면에 실제로 그리는 수. 나머지는 숫자로 (폰이 버텨야 한다)
    approach: 4.2,      // 문이 다가오는 시간(초) — 세 갈래를 읽고 고를 만큼은 줘야 한다
    ...
    shotTime: 0.7,      // 총알이 길 끝까지 날아가는 시간(초)
    shotEvery: 0.5,     // 병사 하나가 이 초마다 한 발
    ...
    gold: { win: 40, perTroop: 0.4, lose: 8 },
    exp: { win: 36, lose: 10 },
  },
